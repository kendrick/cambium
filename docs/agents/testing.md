...
The criterion bites hardest where fixtures are expensive to change and where the output is a classification rather than a number. #105 generates the image fixtures the whole browser tier runs against, in `e2e/fixtures/`. Any ticket producing a derived classification—provenance, a contrast verdict, a role assignment—has the shadow's shape: two rules can agree on every input a fixture happens to carry, and only a fixture chosen to separate them says which rule is running.
...
Playwright is the only browser runner. Vitest gets no browser tier: it keeps whatever runs Node-side, and more kinds keep arriving. Today that means the pure core, the RecordStore contract under `fake-indexeddb`, the bundle measurement, `package-scripts.test.ts`, which spawns `pnpm` as a subprocess and costs seconds where the rest cost milliseconds, and `scripts/{codex-reader,demo-fixture,magick-codec}.test.ts`, which cover the `fixture:demo` generator's own subprocess calls (`codex exec`, `magick`) the same way—mocked in most of those tests, but real in the ones that need a real `magick` on PATH, which skip rather than fail when it's absent. The `cat.color` rule family the interface audit needs is inert under jsdom, which does no layout and resolves no cascade, and run 4 exercises real IndexedDB and real file handling that `fake-indexeddb` cannot prove. Once a browser is running for both, `@axe-core/playwright` covers the audit in the same suite and a second runner earns nothing. Static export keeps the cost down: build, serve `out/` as files, point a browser at it.
